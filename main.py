    ...
    try:
        ...
        try:
            if (req.json()["status_code"] == 0):
                ...
                TotalSendedShare += 1
                if DebugMode == True:
                    print(Colorate.Horizontal(Colors.blue_to_red, f"Poslan share: {TotalSendedShare}|"))
                else:
                    print(Colorate.Horizontal(Colors.blue_to_red, f"Poslan share: {TotalSendedShare}| "))
            else:
                pass
...
if (__name__ == "__main__"):
    ...
    itemID = Write.Input("Link videa: ", Colors.blue, interval=0.0001)
    amount = Write.Input("Količina shareova: ", Colors.yellow, interval=0.0001)
    NThread = 500
